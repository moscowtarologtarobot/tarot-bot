...
async def handle_callback(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if query.data == "free_card":
        ...
        context.args = []
        await query.message.reply_text("Запускаю бесплатную карту…")
        class Dummy:
            pass
        # Call logic inline to avoid fragile object mutation.
        user_profile = upsert_user(update.effective_user)
        if user_profile.free_reading_used:
            await query.message.reply_text("Бесплатная карта уже была. Но платный расклад ещё здесь: /buy")
            return
        card = random.choice(list(CARD_MEANINGS.keys()))
        meaning = CARD_MEANINGS[card]
        text = (
            ...
            "Хочешь глубже — бери платный расклад через /buy."
        )
        mark_free_reading_used(update.effective_user.id)
        save_reading(update.effective_user.id, "basic", "Общий запрос", [card], text, True)
    ...
    if query.data == "my_readings":
        rows = get_latest_readings(update.effective_user.id)
        if not rows:
            await query.message.reply_text("Пока раскладов нет. Начни с /free или /buy.")
            return
        pieces = ["Твои последние расклады:\n"]
        for row in rows:
            created = row["created_at"].split("T")[0]
